...
description: 对固定机位攀岩/抱石视频做本地 GPU 姿态追踪与生物力学分析，渲染竖屏叠加标注成片（骨架/重心轨迹/接触检测/三点平衡状态机/支撑三角/重心转移事件/技术动作面板/语音字幕）。触发：用户提到「攀岩视频 姿态分析」「三点平衡 重心转移」「抱石 技术分析 骨架标注」「攀岩 CoM 分析成片」，或要求分析攀爬动作、给攀岩视频叠加技术标注与字幕。不适用：移动机位跟拍；需要 3D 关节力矩/肌肉力的科研场景（→ OpenSim/Pose2Sim）。
---
...
**视觉层级（三点平衡主线是主角，标注让位于人）**：
- 标注整体克制（用户 2026-08-28 再调淡，勿回浓）：骨架 0.13 纯背景、
  关节点 0.20·r3.5；**不显示关节角度标注**
- 支撑三角为主角但仍透：描边 6·alpha 0.55、fill 0.10、回声层 0.14·宽 10；
  3pt 接触点亮绿脉动圈（alpha 0.85·线宽 4）；移动肢体=虚线暗白圈 0.35；
  其余红圈 0.30；重心 r13·光晕 0.10、轨迹 0.45、垂线 0.20
- **主成片不渲染跟随技术徽章**（用户 2026-08-28 移除：遮挡手脚、跟随滞后、
  与底部面板重复）——技术动作只显示在底部面板；tech 数据仍经 `--tech`
  接入，驱动面板「当前技术动作」与片尾小结卡。讲解片模板暂保留徽章
  （见 TUTORIAL.md），若用户要求同样移除按本条办理
...
- 事件字幕：3pt→3pt 链标注「三点平衡转移」，其余「重心转移」；排队不丢弃；
  单条最长 5s（2026-08-29 用户：读者看不过来——下一个事件到来时交接钳制
  仍及时缩短，无事件则多留一会儿）
